...
- **Repository:** `MilaArtyNew/pump-dump-bot`
- **Primary stack:** Python, systemd, Railway
- **Entrypoints and scripts:**
  - `main.py`
- **Notable dependencies:** `aiohttp`, `python-dotenv`

## Configuration

Configure the service with environment variables. Do not commit real secrets to the repository.

- `CHAT_ID` — required or optional runtime configuration. See deployment environment for the actual value.
- `MIN_PUMP_PCT` — required or optional runtime configuration. See deployment environment for the actual value.
- `MIN_VOLUME_USDT` — required or optional runtime configuration. See deployment environment for the actual value.
...
```bash
git clone https://github.com/MilaArtyNew/pump-dump-bot
cd pump-dump-bot
python3 -m venv .venv
